...
if ($_POST && isset($_POST['luser'])) {
    if (!$_POST['luser'])
        $errors['err'] = __('Username atau alamat email tidak valid!');
    elseif (($user = UserAuthenticationBackend::process(trim($_POST['luser']),
        ...
        if ($user instanceof ClientCreateRequest) {
            if ($cfg && $cfg->isClientRegistrationEnabled()) {
                ...
                $errors['err'] = __('Akses ditolak, Hubungi administrator pusat bantuan Anda untuk mendaftarkan akun untuk Anda.');
        ...
        $errors['err'] = sprintf('%s - %s', __('Username atau password salah'), __('Mohon coba lagi!'));
    }
    $suggest_pwreset = true;
}
elseif ($_POST && isset($_POST['lticket'])) {
    if (!Validator::is_email($_POST['lemail']))
        $errors['err'] = __('Alamat email atau nomor tiket tidak valid!');
    elseif (($user = UserAuthenticationBackend::process($_POST['lemail'],
        ...
        if ($ticket) {
            ...
            $msg = sprintf(__("%s - Tautan akses terkirim ke alamat emailmu!"),
                Format::htmlchars($user->getName()->getFirst()));
            $_POST = null;
        } else {
            $errors['err'] = sprintf('%s - %s',
                __('Alamat email atau nomor tiket salah!'),
                __('Mohon coba lagi!'));
        }
    } elseif(!$errors['err']) {
        $errors['err'] = sprintf('%s - %s', __('Alamat email atau nomor tiket salah!'), __('Mohon coba lagi!'));
    }
}
