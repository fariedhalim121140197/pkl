...
elseif ($_POST) {
    ...
    if (!$user_form->isValid(function($f) { return $f->isVisibleToUsers(); }))
        $errors['err'] = __('Informasi Client tidak lengkap');
    elseif (!$_POST['backend'] && !$_POST['passwd1'])
        $errors['passwd1'] = __('Diperlukan password baru');
    elseif (!$_POST['backend'] && $_POST['passwd2'] != $_POST['passwd1'])
        $errors['passwd1'] = __('Password tidak cocok');
    else {
        try {
    ...
    if ($errors)
        $errors['err'] = $errors['err'] ?: __('Tidak dapat meregistrasi akun. Silakan baca pesan dibawah');
    ...
    elseif (($addr = $user_form->getField('email')->getClean())
        ...
        $user_form->getField('email')->addError(
            sprintf(__('Email sudah diregistrasi. Apakah anda ingin %1$s masuk %2$s?'),
            '<a href="login.php?e='.urlencode($addr).'" style="color:inherit"><strong>',
            '</strong></a>'));
        $errors['err'] = __('Tidak dapat meregistrasi akun. Silakan baca pesan dibawah');
    }
    elseif (!$addr)
